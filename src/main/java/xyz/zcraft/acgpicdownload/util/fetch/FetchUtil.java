...
import xyz.zcraft.acgpicdownload.util.source.SourceFetcher;
...
public class FetchUtil {
    ...
    public static void listSources(Logger logger) throws IOException, JSONException {
        ...
        if (sources == null) {
            logger.info("No available sources");
        } else {
            int a = 0;
            int b = 0;
            int c = 0;
    ...
    public static void startMonitoring(DownloadResult[] result, ThreadPoolExecutor tpe,
        ...
        Thread t = new Thread(() -> {
            ...
            logger.info(ResourceBundleUtil.getString("gui.download.status.completed"));
            onUpdate.run();
    ...
    public static void printResult(DownloadResult[] r, Logger logger) {
        ...
        if (!mapFailed.isEmpty()) {
            logger.info(ResourceBundleUtil.getString("gui.download.status.failed") + ":");
    ...
    public static ArrayList<Result> fetch(Source s, int times, Logger logger, boolean enableConsoleProgressBar,
        ...
        logger.info(String.format("Fetching pictures from %s ...", s.getUrl()));
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("Fetch").append(" 0/").append(times);
        ...
        for (int i = 0; i < times; ) {
            if (times > 1 && enableConsoleProgressBar) {
                ...
                sb.append("Fetch").append(" ").append(i).append("/").append(times);
                if (failed != 0) {
                    sb.append(" ").append(ResourceBundleUtil.getString("gui.download.status.failed")).append(" : ")
                            .append(failed);
                }
                logger.printr(sb.toString());
        ...
        sb = new StringBuilder();
        sb.append("Fetch").append(" ").append(times).append("/").append(times);
        if (failed != 0) {
            sb.append(" ").append(ResourceBundleUtil.getString("gui.download.status.failed")).append(" : ")
                    .append(failed);
        }
    ...
    public static ArrayList<Result> fetch(Source s, int times, Logger logger, boolean enableConsoleProgressBar,
        ...
        logger.info(String.format(Objects.requireNonNull(ResourceBundleUtil.getString("gui.fetch.info.fetch")),
                s.getUrl()));
        ...
        sb.append("Fetch").append(" ").append(times).append("/").append(times);
        if (failed != 0) {
            sb.append(ResourceBundleUtil.getString("gui.download.status.failed")).append(":").append(failed);
        }
        logger.printr(sb.toString());
